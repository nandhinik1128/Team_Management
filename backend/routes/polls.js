...
router.post('/', verifyToken, (req, res) => {
  if (!['captain', 'vice-captain'].includes(req.user.role))
    return res.status(403).json({ message: 'Only Captain or Vice-Captain can create polls!' });
  const { question, options } = req.body;
  db.query('INSERT INTO polls (question, created_by) VALUES (?, ?)',
    [question, req.user.id], (err, result) => {
      if (err) return res.status(500).json({ message: 'Failed!' });
...
router.post('/:id/vote', verifyToken, (req, res) => {
  ...
  db.query('DELETE FROM poll_votes WHERE poll_id = ? AND user_id = ?',
    [poll_id, user_id], (err) => {
      if (err) return res.status(500).json({ message: 'Failed!' });
      db.query('INSERT INTO poll_votes (poll_id, option_id, user_id) VALUES (?, ?, ?)',
        [poll_id, option_id, user_id], (err2) => {
          if (err2) return res.status(500).json({ message: 'Failed!' });
...
// Delete poll — captain/vice-captain only
router.delete('/:id', verifyToken, (req, res) => {
  if (!['captain', 'vice-captain'].includes(req.user.role))
    return res.status(403).json({ message: 'Only Captain or Vice-Captain can delete polls!' });
